        ...
        <Container style={!isProfileView ? (!isExploreView ? {display: 'grid', placeItems: 'center', paddingTop: '5vh'} : {paddingLeft: '5%', paddingTop: '2%', width: '100vw',  marginLeft: '0%'}): {padding: '5%', width: '100vw', marginLeft: '0'}}>
            ...
            {(isProfileView ? data.created_posts : data).map((post, idx) => {
                return (
                    ...
                    auth.is_on_mobile ? <MFeed openPostModal={() => {handlePostModalOpen(post)}} idx={idx} post={post} username={authContext.user_data?.username} />:<div key={idx} style={{width: 'clamp(1000px, 60vw, 1200px)', height: 'clamp(300px, 40vh,400px)', border: '1px solid ' + defaultBorderColor, borderRadius: '5px', marginBottom: '5vh', display: 'flex'}}>
...
function MFeed({ idx, post, username, openPostModal }){
    ...
    return (
        <div key={idx} style={{width: '100%', marginBottom: '4rem'}}>
            ...
            <PostActionBar post_id={post.id} is_liked={ username ? post.likes.includes(username) : false} is_disliked={username ? post.dislikes.includes(username) : false} n_likes={post.likes?.length} n_dislikes={post.dislikes?.length}  />
...
function PostActionBar({ is_liked, is_disliked, n_likes, n_dislikes, post_id }){
