...
export default async (req, res) => {
    ...
    if(req.method.toLowerCase() === "post"){
        ...
        try{
            ...
            const apiResponse = await fetch(`${BACKEND_ROOT_URL}profile/edit/status-indicator/`, {
                method: 'POST',
                headers: {
            ...
            if (apiResponse.status === 200){
                ...
                return res.status(200).json({success: 'Status Indicator Updated'})
            }else{
                return res.status(apiResponse.status).json({error: 'Can\'t Update Status Indicator'})
            }
